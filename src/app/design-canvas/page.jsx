...
export default function DesignCanvas() {
    ...
    const updateNodeConfig = (nodeId, newOption) => {
        setNodes((currentNodes) =>
            currentNodes.map((node) =>
                node.id === nodeId ? { ...node, selectedOption: newOption } : node
            )
        );
    };
    ...
    const deleteNode = (nodeId) => {
        ...
        setConnections((conns) => conns.filter(c => c.from.nodeId !== nodeId && c.to.nodeId !== nodeId));
    };
        ...
        try {
            await fetch('/api/projects', {
                ...
                body: JSON.stringify({
                    ...
                    canvas_data: { components: nodes, connections },
                    status: "draft",
                }),
            });
    ...
    return (
        <div className="h-screen flex flex-col p-4 gap-4 overflow-hidden">
            {/* Header */}
            <div className="glass-card rounded-2xl p-4 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex-1 flex gap-4 min-h-0">
                ...
                <div className="w-80 flex-shrink-0">
                    <ValidationPanel selectedQPU={selectedQPU} setSelectedQPU={setSelectedQPU} />
                </div>
            </div>
        </div>
    );
